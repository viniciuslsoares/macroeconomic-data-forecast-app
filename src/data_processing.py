    ...
    if DEVELOPMENT_MODE:
        ...
        for country in countries:
            for year in years:
                ...
                if country == 'BRA':
                    row['GDP (current US$)'] = 1.8e12 + \
                        (year - 2000) * 5e10 + np.random.normal(0, 1e11)
                    row['Population, total'] = 200e6 + \
                        (year - 2000) * 1.5e6 + np.random.normal(0, 1e6)
                    row['CO2 emissions (kt)'] = 400000 + (year - 2000) * \
                        5000 + np.random.normal(0, 20000)
                    row['Life expectancy at birth, total (years)'] = 70 + (
                        year - 2000) * 0.2 + np.random.normal(0, 0.5)
                    row['Individuals using the Internet (% of population)'] = min(
                        95, 10 + (year - 2000) * 3.5 + np.random.normal(0, 2))

                elif country == 'CAN':
                    row['GDP (current US$)'] = 1.2e12 + \
                        (year - 2000) * 3e10 + np.random.normal(0, 5e10)
                    row['Population, total'] = 31e6 + \
                        (year - 2000) * 0.8e6 + np.random.normal(0, 0.5e6)
                    row['CO2 emissions (kt)'] = 550000 + (year -
                                                          2000) * -2000 + np.random.normal(0, 15000)
                    row['Life expectancy at birth, total (years)'] = 79 + (
                        year - 2000) * 0.1 + np.random.normal(0, 0.3)
                    row['Individuals using the Internet (% of population)'] = min(
                        98, 50 + (year - 2000) * 2 + np.random.normal(0, 1))

                if np.random.random() < 0.05:
                    indicator_name = np.random.choice(
        ...
        data_frames = _handle_missing_values(
            data, missing_values_threshold, verbose)
...
def _handle_missing_values(data: pd.DataFrame, threshold: float, verbose: bool = False) -> List[pd.DataFrame]:
    ...
    """
    # Remove columns with a percentage of missing values higher than the given (or default)
    # threshold
    data_frames = []
    ...
    for country in countries:
        ...
        columns_to_drop = curr_missing_percentage[curr_missing_percentage >
                                                  threshold].index
        if verbose:
            print(
                f"Columns to be removed by missing values threshold {threshold} for country {country}: {list(columns_to_drop)}")
    ...
    for i, curr_data in enumerate(data_frames):
        ...
        filled_others = others.groupby(
            group_series, group_keys=False, sort=False).apply(lambda g: g.ffill().bfill())
